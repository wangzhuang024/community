...
//        return userMapper.selectById(id);  // 使用Redis
...
//        loginTicketMapper.insertLoginTicket(loginTicket);
        ...
        redisTemplate.opsForValue().set(redisKey, loginTicket); // 将loginTicket对象序列化为字符串存储在redis中
...
//        return userMapper.updateHeader(userId, headerUrl);
        // mysql 和 redis 不能在同一个事务中，  先更新再清理缓存   万一更新失败不用再清理缓存
    ...
    //3. 数据变更时清除数据  （不在redis中改变数据， 麻烦，直接删除，使用时再从数据库加载）
